                        ...
                        <div className="w-full flex-1 mt-8">
                            <div className="flex flex-col items-center">
                                <button style={{backgroundColor: color}}
                                        className={`w-full max-w-xs font-bold hover:bg-${colortheme}-500 shadow-sm rounded-lg py-3  text-gray-800 flex items-center justify-center transition-all duration-300 ease-in-out focus:outline-none hover:shadow focus:shadow-sm focus:shadow-outline`}>
                                ...
                                <button
                                    ...
                                    className={`w-full max-w-xs font-bold hover:bg-${colortheme}-500 shadow-sm rounded-lg py-3  text-gray-800 flex items-center justify-center transition-all duration-300 ease-in-out focus:outline-none hover:shadow focus:shadow-sm focus:shadow-outline mt-5`}>
                            ...
                            <div className="mx-auto max-w-xs">
                                ...
                                {errors.email &&
                                    <p className=' text-xs text-red-700 font-medium block'>L'email est obligatoire</p>}
                                <input
                                ...
                                <button onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}
                                        onClick={handleSubmit(async (data) => {
                                            await authentification(data).then(res => {
                                                const tosave = {
                                        ...
                                        className={`mt-5 tracking-wide font-semibold text-${textColorD} text-gray-100 w-full py-4 rounded-lg  transition-all duration-300 ease-in-out flex items-center justify-center focus:shadow-outline focus:outline-none`}>
                                    <svg className={`w-6 h-6 text-${textColorD} -ml-2`} fill="none"  stroke="currentColor" strokeWidth="2"
                                ...
                                <span className='ml-6 text-xs cursor-pointer' onClick={() => navigate("/")}
                                      style={{color: color}}>Accueil,</span>
                                <span className='ml-6 mb-5 text-xs cursor-pointer text-red-800'
                                      onClick={() => navigate("/forgot-password")}>mot de passe oublié</span>
                            </div>
                        </div>
                    </div>
                </motion.div>
