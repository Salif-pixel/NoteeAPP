...
export class mailerService {
    ...
    async sendRequestedPasswordEmail({recipient,firstName,token}:{recipient:string,firstName:string,token:string}){
        ...
        const { data, error } = await this.mailer.emails.send({
            from: 'Notee <onboarding@resend.dev>',
            to: [recipient],
            subject: 'Réinitialisation de votre mot de passe',
            html: `
              <div style="font-family: Arial, sans-serif;">
                <img src="https://example.com/logo.png" alt="Logo de Notee" style="max-width: 200px;">
                <h2>Bonjour ${firstName},</h2>
                <p>Vous avez demandé une réinitialisation de votre mot de passe sur Notee.</p>
